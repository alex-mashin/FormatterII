...
Modified initially for traditio.wiki by Alexander Mashin:
	...
	* {# #}	-- argument capture	(lpeg.Carg),
	...
	re.string to replace the standard string library.
--]]
...
local re = {
	string = string -- can be replaced with another library.
}
...
-- Nearly complete grammar, except the String rule, which depends on flags.
local exp = { "Exp",
	Exp = S * ( m.V"Grammar"
...
-- Return a grammar generator for plain strings depending on flags.
local function string_matcher (flags)
	local P, S = mm.P, mm.S
	local string = re.string
	local find, gmatch, upper, lower = string.find, string.gmatch, string.upper, string.lower
	if flags and find (flags, 'i', 1, true) then
		return function (str)
			local pattern = P (true)
			for c in gmatch (str, '.') do
				pattern = pattern * S (upper (c) .. lower (c))
			end
			return pattern
		end
	else
...
-- Generate a grammar for re pattern depending on flags.
local function pattern (flags)
	exp.String = String / string_matcher (flags)
	return S * m.Cg(m.Cc(false), "G") * m.P (exp) / mm.P * (-any + patt_error)
end

local function compile (p, defs, flags)
	if mm.type(p) == "pattern" then return p end -- already compiled
	local cp = pattern (flags):match(p, 1, defs)
	if not cp then error("incorrect pattern", 3) end
	return cp
end

local function match (s, p, i, flags)
	local cp = mem[p]
	if not cp then
	cp = compile(p, {}, flags)
	mem[p] = cp
...
local function find (s, p, i, flags)
	local cp = fmem[p]
	if not cp then
	cp = compile(p, {}, flags) / 0
...
local function gsub (s, p, rep, flags)
