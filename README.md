...
- *simple*. The new context will be created by captures. Below are subypes of simple selectors:
  ...
  - *regular expression*. At least, two flavours (standard Lua and LPEG Re) are available, and more can be made available with *[lrexlib](https://github.com/rrthomas/lrexlib)*. Any pair of matching non-space non-alphanumeric characters can delimit a default flavour regular expression, except characters that have a special meaning in the selector syntax (`().*+,\|@`), and except quotes (`'"`), if regular expression flavour is not specified and the default on is to be used (a quoted string without a flavour prefix will be treated as a plain table key). The possible flags include `AiDsxXmUu_`, but some of them may be unavailable to a certain flavour. Lua and Re expression support `i` flag for case-insensitive matching. All regular expression flavours support the non-standard *condense* flag (`_`) meaning that spaces, hypens and underscores will be ignored:
